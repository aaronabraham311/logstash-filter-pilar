...
class Preprocessor
  ...
  # threshold [Float] a numeric value used to determine if a token is dynamic based on its frequency, only used if parse is true
  ...
  def process_log_event(log_event, threshold, parse)
    template_string = nil
    dynamic_tokens = nil

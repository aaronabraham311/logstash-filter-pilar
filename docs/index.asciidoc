...
The regexes variable allows regexes to be instered into the parsing pipeline to capture known dynamic tokens before the log is passed through the pipeline. Increasing the number of known dynamic tokens extracted in preprocessing will make the parsed results more accurate.
